...
class VelocityPositionFilter:
	def __init__(self, pos, t):
		...
		self.delta_pos = np.diff(pos)
		self.delta_t = np.diff(t)
	...
	def create_kf(self, sigma_a, sigma_z, x_0, P_0, offset = 1e-8):
		...
		return kf
	def fit_params(self, method = 'nelder-mead'):
		'''
		...
		def obj(x):
			sigma_a = np.exp(x[0])
			sigma_z = np.exp(x[1])
			x_0 = np.array([x[2], x[3]])
			P_0 = np.diag([np.exp(x[4]), np.exp(x[5])])
		...
		self.sigma_a = np.exp(optim_result.x[0])
		self.sigma_z = np.exp(optim_result.x[1])
		self.x_0 = np.array([optim_result.x[2], optim_result.x[3]])
		self.P_0 = np.diag([np.exp(optim_result.x[4]), np.exp(optim_result.x[5])])
	...
	def fit_filter(self):
		...
		self.kf = self.fit_params()

		self.filtered_pos = np.concatenate(([self.kf.x_0[0]], self.kf.x_updated[:,0]))
		self.filtered_pos_var = self.kf.quad_form(self.kf.H, self.kf.Q) + self.kf.R
		self.filtered_vel = np.concatenate(([self.kf.x_0[1]], self.kf.x_updated[:,1]))
